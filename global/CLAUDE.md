...
**Offline user ⇒ defer to a scheduled chase, don't storm**: when a targeted `ask_*` returns the offline/timeout DEFAULT (user provably unreachable now), do NOT keep re-firing every tick — call `defer_to_chase(...)` (or `make_gate(..., next_chase_ts=...)`) to defer the gate to a future chase + block the matching store gate with the SAME `next_chase_ts`; re-defer if the chase resumes still-unanswered. Canonical: workflow/cosa-voice-integration.md → "Offline user ⇒ defer to a SCHEDULED CHASE".
