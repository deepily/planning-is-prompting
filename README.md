...
- [**task-store-discipline.md**](workflow/task-store-discipline.md) - v1.1 (verified behavior — Krishna E2E receipts + exhaustive edge matrix folded 2026-06-15; §9 server-enforced transition graph, §10 verbatim worked examples, §11 known-gaps pointer): the day-to-day practice for the unified task-store (Phase-2 companion) — "use TodoWrite, the store follows you" (hook-mirrored upserts), when an explicit `task_create` is owed (cross-session obligations, decisions, gates), the receipts discipline (`→done` requires shape-validated receipt refs; `→blocked` requires typed refs + chase timestamp; terminal states), the F3 truth boundary (store canonical for live work, TODO.md = narrative + rendered sections), deterministic query patterns, correlation semantics across rewrites//clear/respawn, and fail-open rules. Syncs with the Lupin Phase-2 hook crew before freeze.
