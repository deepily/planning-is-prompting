...
**⏳ In-flight rename (2026-06-08, Rick + Tiberius):** the service package is being renamed `arbiter_vigilance` → `lupin_arbiter_app`, and the host app `fastapi_app` → `lupin_app`. **B1 go-live is sequenced AFTER the rename lands + tests green** (Rick ruling 2026-06-08, María `7cf3310c`) so the systemd unit, run script, and module paths cut over once, on final names — not mid-rename. Names below are updated to the targets where unambiguous; service/script filenames and line numbers will shift with the rename. Build verified on disk as of this note: Lupin `622fe5c` @ wip-v0.1.8 (R0 flag still `true` = in-process arbiter live, :8001 not actuating).
...
flowchart TB
    subgraph host["Host (out-of-band)"]
        ...
        arbiter["lupin_arbiter_app :8001\n(standalone uvicorn)"]
