...
| **NEW cascade hybrid** | ~7-9 (predicted: ~4-5 foundational escalations at Run 2's 5% rate × ~75 input Q's + ~3-4 structural gates: decomposition, dependency-map, goal-coverage matrix, cascade-complete) | ~5-15 min high-leverage decision-making (10-30 sec/gate via Item #2 spoken-headline doctrine, bundled per Lesson 12) |
...
**Post-Run-3 actuals fold into a third row of the comparison table** (Run 1 partial / Run 2 toy / Run 3 Phase 6C real). Telemetry collected the same way as Run 2 (disk-read on cascade topics; manager-classification posts as audit trail; severity 6-field schema metadata).
