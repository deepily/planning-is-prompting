...
> **Note (2026-02-20)**: This document pre-dates the CURL prohibition. Health check
> examples using `curl` should use `python3 -c "import urllib.request; urllib.request.urlopen('URL')"`
> instead. See `~/.claude/CLAUDE.md` → TESTING & INCREMENTAL DEVELOPMENT → CURL Prohibition.
