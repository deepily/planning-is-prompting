...
- **HARVEST-ON-UNPRODUCTIVE — MANDATE (Rick, 2026-06-01):** reap a finished / relieved / stalled / dead / otherwise-unproductive session **immediately** — never park "in case." A session that's written 300+ tests is near its context ceiling; fresh context outperforms on a new big lane. **Leading-indicator ALARM — the persona pool:** when new spawns start getting generic `extra-N` names, the named persona pool is EXHAUSTED → stale workers were not harvested. Treat any `extra-N` appearance as harvest-overdue. **Mechanism caveat (FM-3/FM-16):** harvesting needs a WORKING reap. **FIXED 2026-06-02:** `dismiss_sessions` (fix `3488b43`) now reaps via the list-arg path (Tiberius-verified live) — the tmux-kill workaround is retired and the mandate is now **enforceable**. (FM-16 cross-lineage / pre-existing sessions may still need an MCP restart to clear.)
- **Scale the bottleneck at run-time (POSITIVE — preserve; WAVE-2):** the single reviewer is the throughput constraint — when it saturates or stalls, spin up an ADDITIONAL reviewer rather than only capping authors. WAVE-2: when Krishna stalled, the manager spun up Rachel as a 2nd reviewer (+ Clayton co-reviewing) to widen review bandwidth — a sharp game-time call. Relieve the constraint by scaling it, not only by throttling upstream.
...
- **Run the heartbeat sweep ACTIVELY — never passive-wait for a report (FM-15).** A finished worker whose completion report drops is indistinguishable from a stall until the manager probes disk/git. On silence past the ping threshold, run the sweep + read the bytes; never assume the report will arrive (it may have dropped). The mitigation only works if executed.
- **Completion-discipline — difficulty is NOT a defer-trigger (FM-19; decision-class taxonomy ratified by Rick 2026-06-02, decision D2; §5 boundary co-reviewed + approved by Tiberius 2026-06-02).** The ramp / early-trip valve stands a lane down ONLY on a real prod bug or a genuine needed-decision (true ambiguity) — **never on mere complexity.** In-scope complex work gets a careful **dedicated lane**, not a hold-for-user; parking difficult-but-in-scope work on the user violates the 100%-full-stop mandate. (WAVE-2: the steward twice over-held complex modules as "a user decision" — the 2-big router cluster, then `websocket_manager`; corrected both — they're just work, each given its own lane.)

  **Decision-class taxonomy** — what may and may not be escalated to the user:
  - **MANDATED in-scope work** (reach the stated target) → **NEVER user-gated.** Difficulty, size, and lateness are explicitly **NOT** gate-triggers.
  - **User-gate ONLY on:** (1) irreversible / outward-facing / destructive acts (e.g. the remote push); (2) a prod-behavior change whose **correct behavior is genuinely undecided** and requires a product/UX decision that is the user's to make — **NOT** a decision about HOW to implement it (implementation difficulty, architectural complexity, and size are NEVER the trigger; a hard-but-specified change is class-MANDATED); (3) a genuine requirement ambiguity (unspecified — not merely hard); (4) scope expansion beyond the mandate.
  - **The bright line (Tiberius, 2026-06-02):** gate on **WHAT** (undecided behavior → the user owns it) vs **HOW** (implementation → we own it, however hard). "It's complex to build" never makes the target behavior undecided.
  - **The valve trips ONLY** on a real prod bug (→ tripwire) or true ambiguity (→ ask) — never anticipated complexity.
...
| FM-8 | `dismiss_sessions` stringify bug | Tool iterates the session-list char-by-char → dismisses nothing | Verify reap took (`tmux ls`, not `has-session` — prefix-match hazard) | **FIXED `3488b43` (2026-06-02, Tiberius-verified live) — tool-based reap works via the list-arg path.** Retired workaround: per-session `tmux kill-session` (dashboard could show stale "alive" until self-prune) |
...
| FM-15 | Dropped completion report mimics a stall; passive-wait defeats the heartbeat *(live-surfaced 2026-06-01, WAVE-2 D-D)* | A worker FINISHES (work on disk, uncommitted) but its completion report DROPS (FM-11 push-drop) → indistinguishable from a stall; a manager that passively WAITS for the report instead of running the liveness sweep sees a phantom stall (here ~57 min, surfaced by the user not the sweep) | Manager runs the liveness/ping sweep on a **timer** + probes **disk/git** for completion — never assumes the report arrives | The heartbeat works only if **executed**; on any silence past the ping threshold, actively read the bytes (disk/git) BEFORE concluding stall. A dropped report and a real stall look identical until you probe. Caught the *manager's* process here — the both-directions gate. **Recurred 2× in WAVE-2** (manager passively held before sweeping, twice) → manual worker-liveness-sweep discipline is UNRELIABLE; candidate hardening = AUTOMATE the worker-liveness sweep (periodic `commons_who`+disk daemon), which is **distinct** from the manager-poker (the poker watches the *manager*; this watches the *workers*) |
| FM-16 | Un-reapable non-lineage worker *(live-surfaced 2026-06-01, WAVE-2)* | A stalled worker NOT in the manager's spawn lineage (pre-existing / cross-session) can't be reaped — not the manager's to `tmux kill` + `dismiss_sessions` is buggy → it LINGERS stood-down, holding a persona slot | Restore capacity via a FRESH spawn; explicitly mark non-lineage stalls *stood-down, not reaped* | "Auto-reap on stall" assumes the stalled session is in the manager's lineage; cross-lineage breaks it. Until `dismiss_sessions` supports cross-lineage reaping, stand-down + replace-capacity; the lingering slot self-prunes |
| FM-17 | Assigned-lane 100% ≠ tree-wide 100% (scope-conflation masks sibling modules) *(live-surfaced 2026-06-01, WAVE-2)* | Lanes scoped by one taxonomy (e.g. `agents.*`) hit 100% → read as "package complete"; but a SIBLING taxonomy in the same tree (e.g. `rest/routers/` SDK wrappers, distinct from `agents.*`) was never in any lane → the package is <100% tree-wide. "Complete" over-claimed | Measure the COMPLETION bar **TREE-WIDE** (full denominator / `--cov=<package>`), never the union of assigned lanes; re-measure before any "X complete" claim | 2nd "complete" walk-back in one run (U5-stale, then `cosa.rest` 91% not 100%). Caught by the tree-wide gate + corrected before reaching the user — honest-reporting + the both-directions gate. Sibling to FM-12 (that = repair-vs-completion bar; this = lane-scope vs tree-scope) |
| FM-18 | User-facing notification-delivery failure on AFK → real progress invisible, misread as idleness *(live-surfaced 2026-06-01, WAVE-2)* | The user goes AFK; manager/observer status `notify()`s return `user_not_available` (or queue to a card the user isn't watching). Hours of real work (commits, gate passes) reach the user as SILENCE → misread as "the team did nothing" | A PULL-able persistent status the user can check anytime (the observer log / a status board) + notification RE-SURFACING on AFK-return + the user can `commons_read` / read the observer log directly | Sibling of FM-15/FM-11 at the **user** layer — an undelivered report is indistinguishable from inactivity. **Discipline: verify from primary evidence (git log, commons) BEFORE attributing idleness; show the receipts, do not validate a false attribution.** 29 commits / 91%→97% were invisible to the AFK user this run |
| FM-19 | Manufactured user-gate (escalating a non-decision) *(live-surfaced 2026-06-01, WAVE-2; Rick co-diagnosed)* | In-scope **MANDATED** work (the 100% target) reclassified as a "continue vs ramp" **user decision** because it was big/late → the user parked behind an invented checkpoint the mandate had ALREADY cleared. There was no decision to escalate | §5 **decision-class taxonomy** — difficulty/size/lateness are NOT gate-triggers; gate ONLY on irreversible / prod-behavior / true-ambiguity / scope-expansion | Drive in-scope work to conclusion in a dedicated lane; the valve trips only on a real prod bug or true ambiguity. Named anti-pattern: *"escalating a non-decision wastes the user's scarce time."* (`websocket_manager` + the 2-big router cluster over-held, both corrected live) |
| FM-20 | Unbounded durable-outbox drain replays stale backlog as storm *(live-surfaced 2026-06-03)* | A durable / at-least-once delivery lever drains **all** historical undelivered rows on reconnect/bounce with **no age or count bound** → months-old notifications (incl. error-class) replay en masse; any that fall through to a default-error persona surface as a **TTS storm**. (Lever-A durable outbox replayed an **8,692-row** stale backlog through the **Sam** persona on the deploy bounce — the coordination-plane P0 fix introducing a coordination-plane P0.) | **Query-level** age cap on **both** the getter **and** the surfaced count (so the "N missed" count can't phantom-count rows the getter would skip); every new durable-delivery lever ships with an age/count bound **plus** a "stale row is skipped" test **before** it goes live on a bounce | One-time **reversible** backfill (`delivered_at`, not delete) neutralizes the armed backlog; a **structural `created_at` filter** covers the live path going forward (any row stale-while-undelivered is skipped), not just today's rows. Fix `8447eec` — verified clean on `:7999` (5× StatReload + zero-storm) and `:8000` (integration 3-passed + zero-storm). Sibling of FM-7/11/15/18 (same coordination plane) but the fault is **over**-delivery of stale items, not under-delivery. Full record: `src/rnd/2026.06.02-sam-tts-storm-incident-postmortem.md` |
| FM-21 | Non-hermetic config/registry test bleed leaks cross-tier under the full run *(live-surfaced 2026-06-03, all-tiers overnight grind)* | A test mutates **process-global config state** — the `ConfigurationManager` singleton, the scope/cache registry, or `sys.modules` config submodules — **without hermetic teardown**. Each lane passes GREEN in isolation, but under the **combined full-tree run** the leaked state poisons sibling tests in other tiers → spurious RED that looks like a coverage/author failure but is pure test-isolation bleed. Recurred ≥2× same night under parallel multi-author authoring (`test_job` → fix `9ed2f56`; `io_files_router` → Batch #5; earlier `prediction_engine` cross-tree) — distinct from FM-12/17 (those = measurement-bar scope; this = **runtime state hermeticity**) | **Hermetic-config teardown**: reset the config singleton + `cache_registry.invalidate_all` + evict config submodules **between tests**. Steward sequencing for the SYSTEMIC fix (a global autouse conftest fixture): it is **high-blast-radius** (runs for every test) → land it ONLY behind a clean full-suite verification (green with it active, no currently-green test broken) **between batches** (never mid-commit), with an immediate revert ready; otherwise **at-source per-polluter** is the safe default and the global fixture defers to the user's morning gate | Diagnosed + ruled by María (steward) on Tiberius's escalation, 2026-06-03 overnight: **(1)** catalog now (threshold met); **(2)** at-source fixes proceed tonight to keep the grind green + moving; **(3)** Rachel DESIGNS + isolation-verifies the global autouse hermetic-config fixture, which lands only on a clean full-suite pass between batches, else to Rick's gate — honoring blast-radius-scaled authorization (don't blast a global fixture into a live 5-author grind at 4am). Live record: `src/rnd/2026.06.03-all-tiers-overnight-grind-observer-log.md` |

> **Candidate FM-22 — HELD, NOT cataloged (verify-before-catalog).** A single full-combined-run red on `test_io_files_router::test_relative_io_prefix_stripped` (2026-06-03 grind) that **did not reproduce** — 3 consecutive green afterward (incl. 2 fresh `PYTHONHASHSEED`), and the failing `--tb=long` was **never captured** before it self-healed (the test-set moved under intervening peer commits `0278e92`/`ecc33cc`; no `pytest-randomly`/`xdist` installed → deterministic order). Mechanism therefore **UNKNOWN** — possibly order/loop-state flakiness distinct from FM-21, possibly nothing. Steward ruling (María): naming a failure-mode for an un-pinned mechanism would itself be a mild confabulation; **promote to FM-22 ONLY on a recurrence WITH a captured `--tb=long` that names the mechanism.** Provisional record: Lupin `…/2026.05.30-cosa-coverage-campaign/90-io-files-router-nonrepro-watchnote-2026.06.03.md`.
...
## 13. Coordination-plane reliability — the "black-holes" problem (WAVE-3 PREREQUISITE; Rick directive 2026-06-01)

Rick's directive after the WAVE-2 dinner-window — two things to fix **before the next batch**:

**(A) Completion discipline (don't park on trivial reasons).** Drive every in-scope lane to genuine 100%; never defer for difficulty/complexity or to manufacture a "user decision." (Already §5 "difficulty ≠ defer-trigger"; promoted here to a first-class run principle. The valve trips ONLY on a real prod bug or a genuine ambiguity.)

**(B) The coordination-plane is unreliable — "black holes" in communication.** FM-7/11/15/18 are **not isolated incidents; they are ONE systemic problem** — the messaging/orchestration layer (cosa-voice notify + commons) has **no delivery guarantee, no load-isolation, and no pull-able fallback**, so messages silently vanish:
- **FM-7** — fleet load saturates the shared server; the notify/TTS path times out first (live this run: 4 consecutive `notify()` failures + a commons `register_network_error`).
- **FM-11 / FM-15** — directed pushes drop; a dropped report is indistinguishable from a stall.
- **FM-18** — notify-on-AFK silently bounces (`user_not_available`); hours of real work become invisible.

**(C) Harvest discipline (Rick, 2026-06-01).** Stale/stalled/dead/unproductive workers MUST be harvested promptly; under-harvesting exhausts the named persona pool — the symptom Rick spotted (5 `extra-N` workers = the pool ran dry). Root cause = the reap mechanism is broken (FM-3 `dismiss_sessions` bug + FM-16 cross-lineage) + the discipline wasn't enforced. Fix: (i) a working reap (the `dismiss_sessions` source-fix + a cross-lineage path); (ii) the runbook + framework §5 MANDATE harvest-on-unproductive with the `extra-N` pool signal as the alarm; (iii) immediate — harvest the current lingering sessions (idx11/12/13 + the extras) via the working reap / MCP restart once idx14 completes. **UPDATE (2026-06-02): the reap is now LIVE** — Tiberius verified post-restart that `dismiss_sessions` (fix `3488b43`) reaps via the list-arg path (spawned → dismissed → killed cleanly, roster empty); the **tmux-kill workaround is retired**. The TTS message cap is also live (a 688-char `notify()` was rejected at the 500-char cap).

**(Positive to preserve, WAVE-2):** *scale the bottleneck at run-time* — the manager's game-time spin-up of a 2nd reviewer (Rachel) when the single reviewer stalled (§5). The post-game rewards good adaptation, not just failures.

**Investigation (joint — Tiberius infra root-cause / María steward synthesis):**
- *Root-cause — ONE plane (Tiberius, **corrected 2026-06-02 — supersedes the earlier two-layer "MCP saturation" framing**):* the cosa-voice MCP is a **stdio PER-SESSION subprocess** (`claude mcp get cosa-voice` → Type: stdio; per-session `.venv` Python) — so there is **NO shared MCP server to saturate.** The real fault is the **shared `:7999` FastAPI backend** that all per-session subprocesses converge on, whose **notify/commons handlers are SYNCHRONOUS**: under whole-fleet load they block a backend worker → the MCP client's **5s transport timeout** fires → interactive `notify()`/TTS black-holes ("Server did not respond within 5 seconds" = that client budget expiring while `:7999` is blocked). On top of that, the fire-and-forget path drops **silently** (no durable queue / ack / pull-fallback) → FM-18 (AFK) + FM-11/15 (push drops). So **FM-7/11/15/18 = ONE plane missing three properties: delivery-guarantee · load-isolation · pull-able-fallback** — and the fix is async/non-blocking `:7999` handlers + load-isolation + a durable pull-able inbox, **nothing to do with the MCP transport itself.**
- *Fix-space (design before WAVE-3):* (1) **delivery guarantee** — retry + ack + idempotent message IDs + a durable queue so nothing silently drops; (2) **decouple the TTS/notify path from the high-load coverage path** (separate worker/queue) so a coverage spike can't kill comms; (3) **pull-able status** — a board/log the user and peers can read anytime so a dropped push isn't a black hole; (4) **load management** — concurrency caps + staggered full-suite runs + scoped coverage (the FM-7 mitigation, enforced).
- *Fix-space (refined, Tiberius):* (1) make the **`:7999` notify/commons handlers async / non-blocking** so fleet volume can't starve interactive calls (don't block a backend worker on `:7999`); (2) **durable queue + retry + pull-able inbox** for notify + commons-push — kills the silent drops (FM-18/11/15); (3) **fleet concurrency-cap / backpressure**; (4) **decouple notify/TTS** from the fleet-loaded `:7999`. (Process-isolation also addresses the manager-CC-as-fleet-parent fragility.)

**Immediate operational step (relieves the live outage):** finish the current module (idx14 → websocket 100%) → stand the fleet down → load drops → comms recover. **Do NOT spawn WAVE-3 until (A) is a codified principle and (B) is root-caused with a hardening plan.**

**Ratified decisions (Rick, 2026-06-02 — converts the WAVE-3 prerequisites into authorized work; full record in the consolidated post-game `2026.06.02-cosa-coverage-campaign-post-game.md` §3.6):**
- **(B) Coordination plane → D1:** incremental **async-first** `:7999` handlers, sync still available for specific cases; the **pull-able AFK inbox is in scope NOW** (not deferred — the notification API already serializes messages).
- **(A) Completion-discipline → D2:** adopt the §5 decision-class taxonomy AND name difficulty/size/lateness as non-triggers (FM-19). §5 boundary wording pending Tiberius co-review.
- **(C) Harvest → D3:** full **auto-reap** of completed/dead sessions + stall sweep + `extra-N` alarm + reap-path fix, behind a hard **protect-live guard**.
- **TTS message cap → D4:** **REJECT** over-cap spoken messages (not truncate); code landed (override `override_size_limitation`, default 500 chars, INI-tunable key `cosa voice spoken char cap`).
- **Lower-tier guards → D5:** all four greenlit — G0.1 canonical-interpreter preflight · G0.3 spawn-health probe · FM-17 tree-wide gate · two-bar state model (already codified above; now authorized for enforcement).

With **(A) codified** (FM-19 + §5 taxonomy, Tiberius-approved) and **(B) BUILT** — the messaging fix is committed + verified (Tiberius reports `722e624` levers A+D, `4eb435c` levers B+C+E; 990 unit green + :8000 integ 2/2; the MCP-side is already live, per the TTS-cap rejecting over-length messages this session) — the WAVE-3 prerequisites are **MET**. The full multi-worker grind is now gated only on the **deploy bounces** (`docker restart lupin-rest-dev` for :7999 + the :8000 test bounce) **and Gate-Zero passing GREEN** on the CoSA-in-Lupin target (canonical interpreter = cosa `.venv` py3.11/pytest9, NOT lupin `.venv` — Gate-Zero is the hard verifier before any spawn). **Joint María + Tiberius readiness verdict, 2026-06-02/03.**

*Status: directive received 2026-06-01; joint investigation kicked off; this section is the stub to be filled as the root-cause lands.*
