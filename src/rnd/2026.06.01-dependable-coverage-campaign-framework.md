...
- **HARVEST-ON-UNPRODUCTIVE — MANDATE (Rick, 2026-06-01):** reap a finished / relieved / stalled / dead / otherwise-unproductive session **immediately** — never park "in case." A session that's written 300+ tests is near its context ceiling; fresh context outperforms on a new big lane. **Leading-indicator ALARM — the persona pool:** when new spawns start getting generic `extra-N` names, the named persona pool is EXHAUSTED → stale workers were not harvested. Treat any `extra-N` appearance as harvest-overdue. **Mechanism caveat (FM-3/FM-16):** harvesting needs a WORKING reap — `dismiss_sessions` is bugged (FM-3 → tmux-kill workaround) and cross-lineage/pre-existing sessions can't be manager-reaped (FM-16 → MCP restart); fixing the reap is a prerequisite for the mandate to be enforceable.
...
- *Root-cause — TWO LAYERS (Tiberius):* **Layer A (primary FM-7 driver):** the cosa-voice **MCP server** saturates — it services the whole fleet's tool-call volume (~14 sessions) with handlers that **synchronously block on `:7999` HTTP**, so under load it can't return interactive calls (e.g. `notify()`) within the client's **5s MCP-transport budget** → "Server did not respond within 5 seconds" → drop. (= no server concurrency-isolation / no backpressure. The 5s is the *MCP transport* timeout, NOT a `:7999` POST timeout.) **Layer B (compounds A; code-confirmed):** the blocking notify path (`_send_sync_notification` → POST `/api/notify` on `:7999`) DOES retry/backoff, but each call blocks a server worker on the same `:7999` the fleet loads; the **fire-and-forget** path (`_notify_impl` → `notify_user_async`) drops **silently** (no durable queue / no pull-fallback) → FM-18 (AFK) + FM-11/15 (push drops). So **FM-7/11/15/18 = ONE plane missing three properties: delivery-guarantee · load-isolation · pull-able-fallback.** *(Layer-A = strong hypothesis pending the server concurrency-model read; Layer-B confirmed.)*
...
- *Fix-space (refined, Tiberius):* (1) make the **MCP server concurrent/non-blocking** so fleet volume can't starve interactive calls (async handlers; don't block a worker on `:7999`) — addresses Layer A; (2) **durable queue + retry + pull-able inbox** for notify + commons-push — kills the silent drops (FM-18/11/15); (3) **fleet concurrency-cap / backpressure**; (4) **decouple notify/TTS** from the fleet-loaded `:7999`. (Process-isolation also addresses the manager-CC-as-fleet-parent fragility.)
