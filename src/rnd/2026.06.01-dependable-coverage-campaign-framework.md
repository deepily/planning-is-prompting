...
**"N collected, 0 errors" is COLLECTION, not GREEN.** MUST read actual pass/fail under `<CANONICAL_PYTHON>` and establish a green baseline for a group *before* adding coverage to it. Pre-existing reds are **repair targets** (fix the stale test to match the documented contract — NEVER rewrite an assertion to ratify buggy prod output; escalate genuine prod bugs).
...
**Related Lupin-side ratifications (Rick, 2026-06-01; relayed to Tiberius for execution):** U7 dead-class `ResearchOrchestratorAgent` → **investigate (confirm zero callers) then delete with a resurrection-guard test**. U4 403-queues REST auth → **investigate; arm a tripwire asserting the correct auth contract if it can't be cleanly shown stale** (never ratify a possible real regression as green).
