...
**Status**: v0.4 — **COLD-REVIEWED 2026-06-12**: Clayton cold review **APPROVE-WITH-FINDINGS** (3 receipted passes on `task-store-cold-review`, consolidated C1–C15; manager spot-reproduced receipts; no finding re-opens F1–F4). C1–C5 folded here; C6–C7 = Phase-1 acceptance criteria (§4.1); C8–C15 anchored as Phase-2+ backlog in PIP `TODO.md`. Prior: v0.3 DESIGN-RULED (Tiberius APPROVE qid `b41042a8` + all four forks RULED by Rick, §3.1) — ready for Lupin build (Phase 1, Tiberius crew, post-GCP-cutover)
...
1. **Receipt validation is not theater-able (C6)**: `receipt_refs` enforces a key whitelist (`commit`, `test_run`, `qid`, `doc_path`, `log_line`) with per-key shape rules — `commit` = 7–40 hex chars; `qid` = uuid charset; `test_run` = run-id format; `doc_path` = exists-in-repo check. A non-empty-but-junk receipt (`{doc_path: "trust me"}`) is REJECTED.
...
- **v0.4 (2026.06.12, María)** — **Clayton cold review folded** (APPROVE-WITH-FINDINGS, 3 receipted passes on `task-store-cold-review`, consolidated C1–C15 to Tiberius 03:40:48Z; fold requested qid `fef1f7fa`): **C1 [HIGH]** `correlation_key` schema field + upsert/derivation mechanism (§2.1) — poured in Phase 1; **C2** async-handlers gate pinned staffed→**LANDED** (§3.1 F2, §4 Phase-1, §5 — strengthens the ruled rider; Clayton+Tiberius+María concur on double-wedge evidence; flagged to Rick for veto); **C3** implementation-substrate paragraph naming the SQLAlchemy/Alembic/BaseRepository/`get_db()` stack (§2.1); **C4** debt-clean endpoint mandate (§2.2); **C5** MCP wrappers = cosa-voice-repo deliverable with named owner (§2.2 + §4); **C6–C7** promoted to §4.1 Phase-1 acceptance criteria (receipt key-whitelist/format rules; hook-side auth via `require_api_key_or_jwt`) + full-test-pyramid AC per Rick's voice directive; **C8–C15** anchored as Phase-2+ backlog in PIP `TODO.md`; naming nits fixed (`blocked_on`→`blocked_by` in §2.3; phantom `payload` column removed from §2.1); P2-3 write-side spool/replay steer added to §5.
