...
### 🛑 KISS · Say 3LoL · NoMC C2C · NoAA — the mandate now covers the TERMINAL too (2026-07-19)
...
2. Call `defer_to_chase( pending_user_gates, gate_id, next_chase_ts )` on the hold artifact (or build the row via `make_gate( …, next_chase_ts=… )` for a not-yet-tracked gate) and **rewrite the hold** so the deferral survives turns + `/clear`. This also resets the re-ask budget (`reask_count → 0`).
3. **Block the STORE operator gate to match** — transition it `blocked` with the SAME `next_chase_ts` + `blocked_by=[{kind:user, id:<name>}]`, so the Stop-hook poke, the human UI card, and the arbiter chase-clock all agree on when to resume.

Effect: `is_chase_deferred` suppresses the poke until the chase arrives — the every-turn storm is gone AND the decision is not buried (it is durably queued with a scheduled resurface). **Post-chase RE-DEFER rule (load-bearing):** when the chase arrives the gate resumes cadence-eligible and you re-fire ONCE; if it times out AGAIN (still offline), you MUST **re-defer** (`defer_to_chase` with a NEW future `next_chase_ts`) — because after a *past* chase `is_reask_capped` is False (a chase is set), so a resumed-but-unanswered gate storms every tick again unless the reflex re-applies. Belt to that suspenders: an *un-deferred* gate is still capped at `reask_cap` re-asks then goes quiet, and the arbiter aged-backstop resurfaces a long-quiet gate — but the polite primary is the scheduled chase, not the cap. Full doctrine + the code verbs (`defer_to_chase` / `bump_reask_count` / `is_chase_deferred` / `is_reask_capped` in `src/lupin_cli/claude_code/hooks/lib/heartbeat_user_gates.py`): lupin `src/rnd/v0.1.9/2026.07.03-notify-turn-hold-fix-design.md §8`.
