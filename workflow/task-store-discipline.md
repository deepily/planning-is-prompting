...
**Title hygiene (MANDATE — title = one imperative line; detail → `body`).** A task's `title` is a short, one-line imperative LABEL (~one phrase), NOT a description field. All descriptive / context text — provenance, options, rationale, repro steps — goes in `body`. Paragraph-length titles are an anti-pattern: they wreck the terse board glance (`task_query(terse=True)`) and the `/plan-decide` framing, both of which surface the title alone. **Rollout is convention-forward, not a big-bang re-cut**: write new rows short; backfill an over-long title opportunistically when its row is next touched (a wholesale re-titling pass is just churn). Optional soft-enforcement — a length warning at the write seam (`task_create`), flag-not-block — is a deferred lupin-side follow-on, not a precondition for the convention. (This very doc's rows model it: e.g. `47ba26fd` carries a short title with all detail in its body.)
