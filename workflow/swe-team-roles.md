...
> **You MUST maintain an explicit, current TODO list of the work you own — and keep it honest, turn by turn.** This is not bookkeeping for its own sake: the heartbeat work-owed oracle **reads your list as a sign of life.** An owned item that is *in-progress* or *unstarted* is *work owed* → if you stop with owed work on the list, the arbiter pokes you to keep going. A list that's stale (done items left open, finished work unmarked) lies to the oracle and either nags you for phantom work or lets you go dark with real work pending.
> - **The list is your native TODO tool** (the visible one Rick expects anyway). You don't maintain a second artifact — a `PostToolUse(TodoWrite)` hook **auto-mirrors** your list to a host-readable per-session file (`~/.claude/manager-todos/<sid>.md`) on every mutation, so "just use the list" is the whole discipline. The mirror is keyed on your stable session id, so it survives `/clear`.
