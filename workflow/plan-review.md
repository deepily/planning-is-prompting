# Plan Review (Adversarial + Fitness Gate)

**Purpose**: Two-pass quality gate for implementation plan documents, run **before any code is written**. Pass 1 enforces ownership-language clarity ("done" never claimed without AI-executed verification). Pass 2 enforces design-completeness (every step implementable by a competent-but-unfamiliar engineer without asking clarifying questions). A short REUSE pre-pass runs first to catch accidental redundancy.
...
**Origin**: Pattern lifted from Lupin v0.1.7 CJ Flow async multi-lane milestone (`<lupin>/src/rnd/v0.1.7/2026.04.23-cj-flow-async-multi-lane/05-` and `/06-`). Phases 1–3 of that milestone landed cleanly with no rework rounds attributable to ownership or completeness gaps in the design docs.
...
If any convention is missing, the review is calibrated wrong: stop and amend the docs first, or log a skip-with-reason in `00-index.md` so Pass 1 exempts the missing convention.

---

## 3. Pre-pass: REUSE Detection
...
ownership or completeness review, do a REUSE PRE-PASS.
...
## 4. Pass 1: Adversarial Review (Ownership Language)
...
user will do this step." {{ANCHOR_FILES}} are authoritative; every other
doc must conform to them.
...
1. **Snapshot the pre-fix grep output** (Pass 1 already produced this; save it as the diff baseline).
2. **Apply approved fixes** as edits to the plan docs. Apply only what the user approved.
3. **Re-run the same three greps** against the post-fix docs.
4. **Diff against the baseline**:
   - Pre-existing hits that were targeted by approved fixes should now be gone.
   - New hits should be zero. If a fix introduced a new flag (e.g., new `EXECUTOR: HUMAN` without justification), the fix is incomplete — return to step 2.
5. **Convergence check**: when the diff shows zero new hits AND every approved fix is reflected, the loop closes.
6. Only then advance to Pass 2.
...
## 7. Pass 2: Fitness-to-Implement Review (Design Completeness)
...
```
Now do a FITNESS-TO-IMPLEMENT REVIEW of the same docs you read in the
previous pass. You do not need to re-read the docs — use the context from
the previous pass. Goal: find places where the design is not yet detailed
...
I'll review and decide which findings to resolve before implementation
begins.
```
...
- "Design concerns" section (mandatory in Pass 2, even if empty — say so explicitly)
...
Then return to the Resolution Loop (§6) — apply approved fixes, re-run the two Pass 2 greps against the baseline, confirm convergence.
...
## 10. Layer-3 Design Concerns: Closing the Loop
...
| Reading order ignored | "Do not skip. Do not skim." The anchor files (Layer 1/2) MUST be loaded before the docs being reviewed; otherwise Pass 1 has no anchor to compare against. |
...
**In `~/.claude/CLAUDE.md`**:
- `TEST OWNERSHIP MANDATE` — Layer 1 anchor for Pass 1 ("user is never a tester")
...
The prompts in §3, §4, and §7 contain double-brace placeholders that per-project wrappers fill in. Reference table:

| Slot | Filled with | Used by |
|------|-------------|---------|
| `{{MILESTONE_NAME}}` | Human-readable milestone name (e.g., "CJ Flow async multi-lane") | REUSE pre-pass, Pass 1 |
| `{{BRANCH_NAME}}` | Active branch (e.g., `wip-v0.1.7-...`) | Pass 1 intro |
| `{{ANCHOR_FILES}}` | Ordered list of Layer 1 + Layer 2 files (e.g., `~/.claude/CLAUDE.md` + `00-working-contract.md`) | Pass 1 |
...
| `{{TBD_QUESTIONS}}` | Numbered enumeration of currently-flagged TBDs in the docs (per-milestone, fills at invocation time) | Pass 2 |
...
- `<lupin>/src/rnd/v0.1.7/2026.04.23-cj-flow-async-multi-lane/05-adversarial-review-prompt.md` — Pass 1 instance
- `<lupin>/src/rnd/v0.1.7/2026.04.23-cj-flow-async-multi-lane/06-fitness-review-prompt.md` — Pass 2 instance

Both files are headed `[DONE, DO NOT REEXECUTE]` and parametrized to that specific milestone. This canonical doc is the abstract version; per-milestone wrappers fill in the slots.
