...
**Status**: v1.8 (2026-08-17) — adds the ephemeral `tmp` slot (§3.0b): mementos you are content to lose at reboot write outside the repo to `$LUPIN_MEMENTO_DIR || /tmp/mementos`, no mirror, no gitignore; `io`/`root` unchanged. v1.7 (2026-07-13) — the overwrite RULE is deleted; a MECHANISM replaces it (§3). Writes go through `workflow/scripts/memento_io.py`, never a hand-`Write`.
...
2. **Write the memento** — the full §2 8-element contract, to the §3 location. The location is **always derivable, never handed to anyone**: `io/mementos/<persona-slug>.md` for a spawned worker about to be dismissed (stable, one slot per persona — see §3.2); `<project>/.claude-memento-<persona-slug>.md` for a self-`/clear` (see §3.1). ⚠️ **NOT the bare `.claude-memento.md`** — that shared name is RETIRED (§3.2), nothing writes it, and `self_respin` refuses a memento found there as stale.
...
### §1.1 🔴 WHEN TO **REFRESH** ONE — THE TRIGGER IS BEING GIVEN SOMETHING, NOT ELAPSED TIME

*(Sam 🎙️, 2026-09-02, from hitting it. Ratified by Mr. Radio 🦉 the same evening.)*

Every trigger above says when to **write** a memento. None says when to **refresh** one that
already exists, and the answer is not "periodically" — it is a specific event that nobody
associates with mementos.

🔴 **A MEMENTO GOES STALE IN ONE DIRECTION THAT MATTERS.**

| what changed since you wrote it | what it does to the memento |
|---|---|
| you **FINISHED** work it lists | merely **verbose** — a successor does redundant checking and loses nothing |
| you were **GIVEN** work it does not list | it **LIES ABOUT WHETHER WORK EXISTS** |

⇒ **The asymmetry is the whole rule.** A memento is not a diary entry; it is the ONLY thing a
successor gets. So a line that was true when written and is false now does not gently age —
**`"nothing to resume, nothing owed"` is the single most dangerous sentence to leave stale,
because a successor who reads it STOPS LOOKING.** Held work is then orphaned by a checkpoint
asserting there was nothing to hold.

⇒ **SO THE TRIGGER IS THE ASSIGNMENT, NOT THE CLOCK.** Refresh when you are handed a task,
when you open a commit you are holding, when you accept a gate — not on a timer, and not
"before the reap", because the reap is exactly what you do not schedule.

⚠️ **AND IT IS THE MOMENT NOBODY THINKS ABOUT THEIR MEMENTO.** Measured: a worker wrote a
clean memento ending *"nothing to resume, nothing owed"*, was assigned a second task four
minutes later, and **neither the worker nor the manager thought about the memento for half an
hour** — while a held commit sat in a worktree the checkpoint denied the existence of. Being
given work is the moment your attention is furthest from the file that records what you owe.

**How to refresh — do NOT retype the whole thing, and do NOT edit the record:**
...
loud.** Editing the record with a raw tool lands your text with a **stale mirror and a stale
pointer** — the two surfaces everything else reads — **and it looks like it worked.**

⇒ **When you amend, say in the new entry that it SUPERSEDES the earlier closing line**, and
point the reader at the current one. Both entries live in the same record, and a successor
reading top-down hits the stale "nothing owed" first.

⇒ **Then verify the POINTER carries it, not just the record.** A record sitting correctly at
its own path while the pointer names something else is the exact failure that broke every
seat's re-spin on 2026-09-02 — see §3.2.
...
**Element 1's header MUST carry the writing session's `session_id` and `written_at`** — they are what make a record self-identifying, and what let a reader tell a live memento from a stale one **without inspecting mtimes**. You do not have to remember this either: `memento_io.py` (§3.1) stamps a machine-readable `<!-- memento-record: persona=… session_id=… written_at=… slot=… -->` as **line 1** of every record, and re-stamps it on every amend. **That line is the record's only identity — do not repeat it in the body** (Rick's ruling (a), 2026-09-06): a body `**Written**:` line freezes at first write while line 1 moves, so an amended memento would show a reader its oldest timestamp first. **Provenance does not depend on the author having remembered it.**

```markdown
# .claude-memento-<persona-slug>.md      <-- root slot; `io/mementos/<persona-slug>.md` for the io slot
...
| **POINTER** | `io/mementos/<persona>.md` · `.claude-memento-<persona-slug>.md` | **MUTABLE — and SAFE to overwrite PRECISELY BECAUSE IT IS NOT THE RECORD.** Regenerable from the directory at any time. |
...
### §3.0b The THREE slots — `io`, `root`, and the ephemeral `tmp` (added 2026-08-17)

There are now **three** slots. The two durable ones are unchanged; `tmp` is new and is the one you reach for when a memento should **not** survive the night.

| Slot | Base directory | Durable? | Mirror | Gitignored |
|---|---|---|---|---|
| `io` | the repo (`io/mementos/`) | yes | yes | yes |
| `root` | the repo (`.claude-memento*.md`) | yes | yes | yes |
| **`tmp`** | **`$LUPIN_MEMENTO_DIR` \|\| `/tmp/mementos/<repo>/`** — **OUTSIDE the repo** | **no — boot-wiped** | **none** | **n/a (outside repo)** |

**Why `tmp` exists** (Rick's ask 2026-08-06, GO 2026-08-17; plan `src/rnd/2026.08.06-ephemeral-runtime-state-to-tmp.md`): stray mementos pile up because cleanup is nobody's job. A memento written to the host `/tmp` is collected by the nightly boot wipe, so **expiry is automatic and cleanup is no one's chore.** It deliberately drops the two durability invariants — **no mirror** (the mirror exists to survive `git clean`; mirroring an ephemeral file rebuilds the clutter this slot deletes) and **no gitignore** (a path outside the repo has nothing for `git check-ignore` to say).

- **Use `tmp`** for a re-spin/self-clear memento you are content to lose at the next reboot — the common case now that overnight carryover is explicitly not wanted.
- **Use `io`/`root`** when you still want a lasting, `git clean`-proof record. They are **untouched** by this change.
- **The R-1 post-game gate is unaffected**: it arms only on `--slot root`, so `tmp` never trips it.

> ⚠️ **GUARDRAIL — files in `/tmp` are fine; a PROCESS rooted in `/tmp` is not.** Writing a memento FILE to `/tmp` moves nobody's working directory. `detect_project()` walks up from **cwd**, so the thing that trips the project-detection alarm is a *session cwd* or a *worktree* in `/tmp`, never a file written there. Do not read "never touch `/tmp`" into this slot — that older, broader warning is about where a session RUNS, not where it WRITES.
...
- **A naive reader is already correct.** `Read io/mementos/sam.md`, `seed_memento=io/mementos/sam.md`, an inherited *"read your persona's pointer and rehydrate"* instruction — every one of them gets the **current record's full content**, with **zero extra action**. ⚠️ **An inherited instruction naming the bare `.claude-memento.md` is the ONE case this no longer covers**: that name is retired and unwritten, so it resolves to nothing or to a frozen leftover. Re-point such an instruction at the per-persona pointer. The §3 governing principle (a derivable path nobody hands you) survives untouched.
- **A reader that wants the record's real path** reads the `<!-- current: … -->` line, or runs `memento_io.py resolve --persona <p> --slot <io|root>`.

> ✅ **BOTH POINTERS ARE PER-PERSONA. The shared `root` pointer is RETIRED as of `00fac2b` (2026-09-02).** `pointer_rel_path` now returns `.claude-memento-<persona-slug>.md`, matching the `io` slot's `io/mementos/<persona>.md`. The legacy flat `.claude-memento.md` is **read-only legacy** — nothing writes it any more, so it can only ever be stale, and **there is deliberately NO fallback onto it**: an ABSENT per-persona pointer means *read your own RECORD*, never *fall back to the shared file*.
>
> ⚠️ **THIS BANNER IS THE DOCUMENT'S ONE VOICE ON THE SUBJECT (Sam 🎙️, 2026-09-02).** Until this sweep it was contradicted by eight passages elsewhere that still presented the bare name as live — and a self-contradicting doc is WORSE than a merely stale one, because a reader who finds this banner concludes the file is maintained and then trusts the stale half. **The banner made the stale instructions more credible, not less.** Every remaining bare `.claude-memento.md` in this file is now deliberate and is one of exactly three kinds: a `.gitignore` line (§3.3 — legacy files still exist on disk and must stay ignored), a HISTORICAL narrative (§3.3's 2026-07-13 account, §7's origin note, the Version History), or a RECORD OF A REJECTED PROPOSAL (§5 v2, kept so nobody re-proposes it). **None of them instructs anyone to write there.**
>
> ⚠️ **THE FALLBACK IS THE TRAP, NOT THE SAFETY NET.** Measured 2026-09-02 in lupin: two seats had a **present-but-stale** per-persona pointer, so the fallback never fired, the header check never ran, and both resolved cleanly to a memento **two sessions old**. A fallback that only fires on absence is silent about staleness — which is the failure that actually happens.
>
> **What SURVIVES from the shared-pointer era — on a DIFFERENT REASON. Do not delete it when you notice the old reason is gone:**
>
> - **A rehydrate instruction should still name the RECORD, not the pointer.** The old reason was cross-persona theft (measured 2026-08-30: two seats took one pointer inside forty minutes), and that is dead by construction. **It still holds because A PERSONA OUTLIVES ITS SESSIONS** — `.claude-memento-<persona>.md` written by one session is followed by the *next session of the same persona*, so a stale pointer names a session that is not the reader's. The RECORD name is unambiguous about **both**.
> - **Visiting a repo you do not live in? Still write `--slot io`.** Not because `root` is shared — it no longer is — but because the root slot is the *resident's* rehydrate path and a visitor has no business in it.
>
> 🔴 **A CORRECT CONCLUSION RESTING ON A RETIRED PREMISE IS HOW SOMEBODY DELETES IT NEXT TIME.** A future reader will be **right** that cross-persona theft no longer happens and **wrong** to remove the instruction. Fix the premise; keep the rule.
>
> 🔴 **THE ORDERING RULE THAT GOVERNED THIS MOVE, KEPT BECAUSE IT WILL GOVERN THE NEXT ONE.** Moving a pointer's NAME moves the writer and **every reader of that name** — this document, `.claude/commands/plan-memento.md`, lupin's copy of that command, the code that resolves it, the prose that explains it, and **the tests that pin it.** Store row `8f5dc4df`: **the commands land WITH or AFTER the writer, never before it.** Between 2026-08-31 and 2026-09-01 the PIP command named a per-persona pointer **no code ever created**.
>
> ⇒ **AND THE MOVE ITSELF PROVED THE ORDERING RULE IS NOT ENOUGH. 2026-09-02: four classes of reader, each found by a DIFFERENT instrument.** CODE readers by the outage · PROSE readers by peer review (four filed, **five actual** — the fifth found by sweeping for the retired premise rather than working the filed list) · **TWELVE TEST readers only by the FULL unit tier**, one of whose names still encoded the retired claim · DOCSTRING readers by a worker reading an `Ensures` clause that overstated its own verification. ⇒ **Prose fails silently forever; tests fail loudly but only under a full tier; neither is visible to a grep for the code.** **Scoped-green missed all twelve.**
...
**v1 (current)** — Manual:
- User points the fresh session at the memento by saying *"read `.claude-memento-<persona-slug>.md` and rehydrate"* — **naming the persona's own pointer, never the retired shared `.claude-memento.md`**
...
| Aspect | `.claude-session.md` | `.claude-memento-<persona-slug>.md` |
|---|---|---|
...
- **Gate-refused findings (READ side, 2026-09-02)**: `session-end.md` §1.6 — harvests findings the ticket flow-ratio gate REFUSED out of element 8 and into `TODO.md`. Rick's ruling: *"If the worker can't amend a current existing row that's related, the finding goes into a given user's memento and at the end of the day goes into the to-do file."* ⚠️ **This gives element 8 a SECOND reader with a different deadline.** The owed-work reconciliation below fires on REHYDRATE; this one fires at SESSION END, and it fires even when the seat is never rehydrated at all. A refused finding is exactly what this element was demoted to carry — *"the intent the store doesn't carry"* — because the store did not merely lack it, it **refused to hold it**.
...
- **v2.1 (2026-09-11, María 🌸 — Rick's ruling (a) of 2026-09-06, row `47f33bba`, plan reviewed by Mr. Radio 🦉)** — **Line 1 is the only identity; the writer no longer injects `**Written**` / `**Written by**` into the body, and the §2 template drops both.** `cmd_amend` re-stamps only the machine header, so the injected `**Written**` froze at first write: 59 of 99 comparable amended records showed a stale timestamp first, median 90 minutes (Tiberius 👑, 2026-09-04). **Only `**Written**` ever went stale; `**Written by**` (persona + session) cannot change under an amend, except through `--allow-foreign-record` on the io slot. It is dropped because the ruling keeps one source, not because it was wrong.** An author's own lines are left as typed. ⚠️ The header is an HTML comment, so a rendered markdown view (the doc viewer) now shows no identity; a raw read shows line 1. Before this there were **zero** tests on the injection; `test_memento_io_body_carries_no_frozen_identity.py` went red on the old writer (2 of 4, the two controls green) and green on the new one, and the full memento suite is 282 passed.
- **v2.0 (2026-09-02, Sam 🎙️ — Mr. Radio 🦉's word)** — **THE DOC STOPPED CONTRADICTING ITSELF: eight passages still presented the RETIRED shared `.claude-memento.md` as the live root pointer, while §3.2's banner said it was retired.** The writer moved at `00fac2b`; these eight sentences did not. **A self-contradicting doc is worse than a merely stale one** — a reader who finds the banner concludes the file is maintained and then trusts the stale half, so **the banner made the stale instructions MORE credible, not less.** Classified by RISK, because instructions and descriptions do not carry the same one. 🔴 **INSTRUCTIONS (a reader following one REPRODUCES the outage)**: §0 beat 2 — *"`<project>/.claude-memento.md` for a self-`/clear`"*, the urgent one, since writing there gets `self_respin` refusing with *"memento is stale"*, which is exactly the defect that produced twelve unit reds this same day; §4's lifecycle row; §5 v1's *"read .claude-memento.md and rehydrate"*, still labelled **current**. **TEMPLATE (copied, therefore instruction-shaped)**: §2's element-1 header block. 🔴 **A CLAIM THAT HAD BECOME FALSE**: §3.2's *"a naive reader is already correct"* asserted that an inherited *"read `.claude-memento.md`"* instruction still resolves to the current record — it does not, and that is now called out as the one case the pointer design no longer covers. **DESCRIPTIONS**: the §3 RECORD/POINTER/MIRROR table, and §6's comparison table twice. ⇒ **The remaining bare mentions are deliberate and are exactly three kinds** — a `.gitignore` line (legacy files still on disk must stay ignored), HISTORICAL narrative (§3.3's 2026-07-13 account, §7's origin note, this Version History), and a RECORD OF A REJECTED PROPOSAL (§5 v2, kept so nobody re-proposes it) — **none of which instructs anyone to write there**, and §3.2's banner now says so, so the file has one voice. ⚠️ **The count arrived as "seven" and was wrong in BOTH directions**: a grep found the NAME and the classification needed the USE. Re-deriving and reading each in context added three genuinely-stale passages (the template, the false claim, §6's second row) and removed two that are legitimate history. **A population picked by name is wrong by default.**
- **v1.8 (2026-09-02, Sam 🎙️ — ratified by Mr. Radio 🦉 the same evening)** — **NEW §1.1: when to REFRESH a memento, which every trigger in §1 was silent about.** §1 says when to WRITE one; nothing said when to refresh one that already exists, and the answer is not a timer. **A memento goes stale in ONE DIRECTION THAT MATTERS**: finishing work merely makes it VERBOSE, while **gaining work makes it LIE ABOUT WHETHER WORK EXISTS** — and `"nothing to resume, nothing owed"` is the most dangerous sentence to leave stale, because a successor who reads it STOPS LOOKING. ⇒ **So the trigger is BEING GIVEN SOMETHING, not elapsed time** — which is precisely the moment nobody thinks about their memento. Anchor, measured rather than reasoned: a worker wrote a clean memento closing *"nothing to resume, nothing owed"*, was assigned a second task four minutes later, and **neither the worker nor the manager thought about the memento for half an hour** — while a held commit sat in a worktree the checkpoint denied the existence of. §1.1 also records the mechanism that catches the naive repair: `write` **REFUSES** a second record (*"A record is IMMUTABLE. Nothing overwrites it, including you"*), names `amend` as the verb, and warns that a raw-tool edit lands text with a **stale mirror and stale pointer and LOOKS LIKE IT WORKED** — refuse-don't-half-finish plus an executable remedy in one message, on a caller who was deliberate and would otherwise have overwritten an immutable record. Adds the two obligations that follow: an amend must say it **SUPERSEDES** the earlier closing line (both entries live in one record and a top-down reader hits the stale one first), and the author must verify the **POINTER** carries the change rather than just the record — the exact failure that broke every seat's re-spin that afternoon.
- **v1.9 (2026-09-01, María 🌸)** — **§3.2 states the `root` pointer is SHARED, and says what to do about it.** The document has 120 mentions of "memento" and, until now, **zero** for the one hazard a multi-persona repo actually hits: at HEAD the `root` slot is SPLIT — `record_rel_path` is per-persona (`.claude-memento-<persona>-<sid>.md`, `memento_io.py:581`) while `pointer_rel_path` returns one flat `.claude-memento.md` for everybody (`:649`). Measured in this repo: **six** personas' root records, **one** pointer — last writer owns it. Two instructions follow (visit a repo ⇒ `--slot io`; name the RECORD, not the pointer, in a rehydrate instruction), plus the **ordering rule** from store row `8f5dc4df`: Step 3 of the per-persona-pointer change (`072ef7e`) is reverted at `d4f6c29`, and re-landing it moves the pointer's name, so **the commands land WITH or AFTER the writer, never before** — between 2026-08-31 and 2026-09-01 `.claude/commands/plan-memento.md` named a per-persona pointer no code ever created. **The gap was found by Mr Radio 🦉 in lupin's copy of the command; his remedy — sync the copy FROM the canonical rather than hand-patch it — required the canonical to carry it first, which is this entry.** Suite green, 425 passed. **HELD for commit.**
- **v1.8 (2026-08-17, Krishna 🦚 — implementing María's approved design, Rick GO)** — **THE EPHEMERAL `tmp` SLOT (approach A; new §3.0b, §3.1/§4 pointers).** A third slot writes the memento OUTSIDE the repo to `$LUPIN_MEMENTO_DIR || /tmp/mementos/<repo>/`, so the nightly boot wipe collects it and cleanup is nobody's chore — Rick's ask 2026-08-06, GO 2026-08-17 (row `154fbf19`). It deliberately drops the two durability invariants: **no mirror** (the mirror exists to survive `git clean`; mirroring an ephemeral file rebuilds the clutter the slot deletes) and **no gitignore** (a path outside the repo has nothing for `git check-ignore` to say). **`io`/`root` are untouched** — still durable, still mirrored, still gitignored, existing callers unchanged (Rick's ruling: add the slot, do not sweep io/root callers). **Approach B (repointing the `root` slot's base to `/tmp`) was rejected on measurement** — it refuses to write, because the repo-relative helpers (`ensure_gitignored`, `mirror_path_for`, `relative_to(repo_root)`) break on an absolute path; approach A keeps them honest via a `slot_base_dir()` seam. The R-1 post-game gate is **unaffected** — it arms only on `--slot root`. Verified by execution: `test_memento_io_tmp_slot.py`, with a **proven-red** falsifiable regression (reverting `slot_base_dir(tmp)`→`repo_root`, or restoring the mirror leg, reddens it), plus a **cross-session read-back** (memento written to `/tmp/mementos/planning-is-prompting/`, read + sha-confirmed by María from a second session). Design: `src/rnd/2026.08.06-ephemeral-runtime-state-to-tmp.md` §4.3. **HELD for commit.**
