...
> 🔴 **THE `root` POINTER IS SHARED BY EVERY PERSONA IN THE REPO — the `io` pointer is not.** Measured at HEAD, 2026-09-01: `record_rel_path` gives `root` a **per-persona** record (`.claude-memento-<persona>-<sid>.md`, `memento_io.py:581`), while `pointer_rel_path` gives it **one flat name for everybody** — `.claude-memento.md`, `memento_io.py:649`. This repo holds root records from **six** personas and **one** pointer, so the last seat to write owns it.
>
> Two consequences, in the order they bite:
>
> - **Writing `--slot root` in a repo you are only visiting re-points the resident's pointer at your record.** Nothing is destroyed — records are immutable — but a naive *"read `.claude-memento.md` and rehydrate"* instruction now hands that reader **your** session. **Visiting? write `--slot io`, which is per-persona.**
> - **A rehydrate instruction should name the RECORD, not the pointer,** in any repo more than one persona works. `.claude-memento-<persona>-<sid>.md` is unambiguous; `.claude-memento.md` is whoever wrote last.
>
> ⚠️ **This split is KNOWN, and it is not yours to "just fix."** Step 3 of the per-persona-pointer change (`072ef7e`) was **reverted at `d4f6c29`**. Re-landing it *moves the pointer's name*, so the writer and **every command that names a pointer** must move together — this document, `.claude/commands/plan-memento.md`, and lupin's copy of that command. Ordering rule, from store row `8f5dc4df`: **the commands land WITH or AFTER the writer, never before it.** Between 2026-08-31 and 2026-09-01 the PIP command named a per-persona pointer **no code ever created** — that is the exact failure this rule exists to stop.
