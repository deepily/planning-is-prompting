...
**Dev-server bounce — a STANDING manager prerogative (Rick, 2026-07-06; generalizes the 2026-06-24 arbiter-fix carve-out).** Bouncing the **arbiter server (`:8001`)**, the **test server (`:8000`)**, or the **notification server `lupin-rest-dev` (`:7999`)** is the **manager's prerogative under standing authority** — no separate user gate, for **any** reason (deploy a fix, clear state, pick up new config, recover a wedged service), **provided the single predicate holds: there is nothing currently running on that server** (no in-flight job, test run, migration, or live request — the idle/quiesce check). Hygiene: **(a) announce + log** the bounce (post-hoc visibility, §5) and **(b) roll back / re-bounce** if it regresses. *Rationale:* a dev-infra bounce on an idle server is reversible and low-blast-radius; gating it behind the user only adds latency to routine fleet ops, and two managers with this prerogative shouldn't have to wake the user to restart their own dev servers (Rick's 2026-07-06 broadcast: *"Why am I a gate on a process in which 2 managers … have the prerogative to bounce the server whenever it is needed?"*). This absorbs the older **owner-scoped arbiter-fix** carve-out (v1.5) and the Tester's `:8000` *bounce-if-idle* precedent (`swe-team-roles.md`) into one rule: **idle dev server → standing bounce**. **Scope guard — what stays STILL-GATED:** (1) bouncing **while a job/test/migration is running** (wait for idle, or get the user's direct word to interrupt it); (2) bouncing an **outward-facing / production** service, or a **non-dev** shared service; (3) a **flag-flip / put-into-service / deploy-to-prod** (a different, outward-facing act — never a mere bounce).

#### `:7999` is standing too — with a wider idle check and a mandatory warn (2026-08-01)

> 🔑 **`:7999` is the one server ANY seat may bounce — not just managers (Rick, 2026-08-01, by keypress).** Every other line in this section is scoped to manager-role sessions; this one is not. His words as relayed: *"anybody may bounce `:7999` within reason to pick up fresh code."* **Why it widened**: with auto-reload off, every change to served code needs a bounce to land, and a worker who cannot bounce cannot finish. **Use the script** (below) — it is the sanctioned path and it brackets the outage for you; if you bounce by hand instead, the warn and the idle check below are still yours to satisfy.

**`:7999` is the fleet's own comms channel**, so its bounce differs from `:8000`/`:8001` in blast radius, not in tier. Restarting it wipes the WebSocket registry and the pending-ask table: **every session's in-flight blocking question dies, and the user's queued notifications go with it.** Two rules follow, and neither is a user gate:

1. **The idle check is fleet-wide, not local.** For `:8000`/`:8001`, *idle* means nothing is running on **your** server. For `:7999` it means **no session anywhere is holding an open blocking ask**. Your own board being quiet is not the predicate.
2. **Warn first, all-clear after — always.** A bounce nobody was warned about is indistinguishable, from every other seat, from the server having crashed. **You no longer send either half by hand** — use the sanctioned script (below), which brackets the outage for you.

**Use the sanctioned script: `lupin/src/scripts/bounce-dev-server.sh`** (committed `db394fc7`; `--quiet` for a one-line summary). It warns, restarts, and polls `/health`, and it is the discoverable form of the two rules above:

- **The warning waits for delivery, not for a 200.** `bounce_dev_warn.py` posts with `require_ack=True` and then polls `broadcast-acks` until every recipient acks or an 8-second deadline passes. It deliberately refuses to treat the HTTP 200 as proof — the server never awaits its own push future, so a 200 can race the restart and warn nobody. Partial reach exits 1 and says *"warned N of M"* rather than proceeding quietly.
- **The all-clear comes from the restarted server, not the script** — it fires from the startup hook once sockets have actually rejoined, so it covers **every** restart path, including a raw `docker restart` somebody ran by hand. There is deliberately no `SIGTERM` handler (installing one would replace uvicorn's and break graceful shutdown); the lifespan block already runs on a graceful stop. Best-effort — it loses to `docker kill`, and the next boot's all-clear closes the loop anyway.

> ⚠️ **Restart ≠ force-recreate, and picking the wrong one applies nothing silently.** Mounts and environment variables resolve at container **create** time. `docker restart` (what the script does) serves new Python from the bind mount, but a change to `docker-compose.yml`, a mount, or an env var needs **`docker compose up -d --force-recreate lupin-rest-dev`** — a longer outage, and the only thing that will re-arm `LUPIN_RELOAD`. A plain restart after a compose edit leaves you looking at old configuration with no error to tell you so.

**A saved file is not a served file (the consequence of R1).** Auto-reload on `:7999` is **off** as of 2026-08-01 — it is opt-in via the `LUPIN_RELOAD` environment variable, gated by `reload_enabled()` in `bootstrap_helpers.py`, and verified empty in the live container. So an edit to a served source file **does nothing until somebody deliberately bounces**. Years of live-reload trained the opposite reflex, and it has already cost us once: a ruling that was correct on disk went unloaded for hours while the running process kept using the old code, and every failure it produced was recorded as data. **If you changed code that `:7999` serves, landing it is your job — bounce, or say plainly that you did not.** *(Founding incident 2026-08-01: a manager sat idle rather than bounce, because this file granted the prerogative for `:8001` and `:8000` and did not mention `:7999` at all.)*
...
1. **IMPLICIT (standing): the repo's declared managers.** Each repo declares its managers on one line of `~/.claude/fleet-roster.env` — `COSA_VOICE_MANAGERS__<PROJECT>="<names>"` (e.g. Mr. Radio and Cheech for Lupin, María for planning-is-prompting) — and the persona chain that spins them up first is DERIVED from that line, so the roster is the only place a manager is named (changed 2026-08-18; before that a hardcoded chain and the roster both claimed the answer and drifted). That persona is **automatically a manager-figure** for its repo — it carries this standing spawn/harvest autonomy, the explicit-TODO discipline, and the work-owed poke treatment **by construction**, with no separate "list me as a manager" step.
...
**Enforcement (doctrine + teeth — BUILT 2026-06-22):** doctrine alone does not prevent this (a manager who knows the model still defaults to the convenient Agent tool — see the founding incident). So the rule is backed by a **`PreToolUse` hook that DENIES the Agent/Task call for a crew-manager session** and redirects to `spawn_sessions`. The hook detects a crew-manager via **EITHER of two role signals** (Rick-ruled): **(1)** a non-empty **spawn manifest** (`~/.claude/sessions/spawned-<id>.json` — the session has spawned workers); **OR (2)** the session's persona is one of the standing manager-figures DERIVED from the fleet roster — the union of every `COSA_VOICE_MANAGERS__<PROJECT>` line plus every named element of the `COSA_VOICE_PREFERRED_PERSONA__<PROJECT>` chains, canonical-matched for accent/case. (`LUPIN_MANAGER_PERSONAS` was a hand-set list and is RETIRED — the hook reads the roster so there is no second place to update.) Signal (2) catches the founding case the manifest alone misses: an *ad-hoc* manager-figure who only ever uses subagents and never spawns. The hook is **fail-open** (any error → allow; a hot-path check never breaks a tool call) and **default-OFF** (gated on the `LUPIN_SUBAGENT_GOVERNANCE` env flag — unset → inert). A session matching neither signal (e.g. a solo builder using read-only Explore) is exempt. *(Built in lupin: `src/lupin_cli/claude_code/hooks/lib/subagent_governance.py` + wired into the existing `pre_tool_use.py`; Mr-Radio-reviewed. This doctrine is the portable rule. Activation = set `LUPIN_SUBAGENT_GOVERNANCE` in the session launch env; the manager list needs nothing set — it comes from the fleet roster.)*
...
· raise ANY row to P0 (operator ALONE — a firewall, not a permission check; managers petition like anyone else)
...
*Version 2.1 (2026-09-07, Tiffany 💍 — Rick-approved, 2026-09-07) — **§2 STILL GATED gains "raising any row to `P0`"**, beside push-to-origin. The STANDING tier implied a manager may set any field on a row it owns; `P0` is the exception. The rule is a **firewall, not a permission check** — no path sets `P0` without an operator identity, managers included — so a manager holds the same **petition** a worker holds, not a lesser version of the operator's power. ⚠️ **Unenforced in code today** (one partial, status-only role check exists fleet-wide); until a check lands, the authorization for any raise belongs in the row. Canonical: `priority-pull-policy.md` §7 *The value space and who may set it*.*
