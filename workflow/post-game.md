...
- **TWO TRANSPORTS, TWO JOBS (2026-07-17, store `a24f27c0`): the window-OPEN and the `harvest-complete` ack are BLOCKING signals and MUST also be `dm_send`'d to the Manager (push transport) — the topic post alone is not delivery.** The topic remains the durable record (it survives the reap — that is why contributions go there); the DM is what makes the gate *resolvable*. Founding incident: a Manager held a reap on five seats waiting for an ack that sat on the commons blackboard he had no reason to poll — *"a blocking gate must not resolve on a surface with no push."*
