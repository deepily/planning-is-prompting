...
### Task-item ledger (added 2026-07-12)

The Manager's stepwise task-store ledger — the cascade's external progress surface. Canonical spec: `plan-review-cascaded-common.md` §Stepwise Task-Item Ledger.

| Key | Default | Rationale |
|---|---|---|
| `task_ledger_required` | `true` | Whether the Manager MUST mint the ledger before the cascade fires. `false` only for throwaway/experimental runs — a cascade a user might ask about is never throwaway. |
| `task_ledger_tiers` | `step_plus_section` | Granularity. Allowed: `step_plus_section` (one item per cascade step, minted at Step 0 + one per section, minted at Step 2 close — the default; step tier alone is a checklist, section tier is what shows the pipeline advancing), `step_only` (coarse; loses per-section motion), `section_only` (loses step-level state — not recommended, the Manager's own rehydration depends on the step tier). |
| `task_ledger_correlation_key` | `cascade-<slug>` | REQUIRED, identical across every item in the run. Without it the run cannot be pulled as a unit: `task_query(correlation_key="cascade-<slug>", terse=True)`. |
| `task_ledger_stale_threshold_minutes` | `20` | How long a ledger item may sit in a status that contradicts observable cascade state before the Steward (or arbiter) flags it stale. A ledger nobody updates is worse than no ledger — it lies. |
...
| `task_ledger_ready` | manager | Step 0.4 output — the stepwise task-item ledger's step tier is on the board (one item per cascade step, shared `correlation_key`, non-null `accountable_manager`). Body carries the correlation key so any reader can pull the run as a unit. The section tier follows at Step 2 close. See `plan-review-cascaded-common.md` §Stepwise Task-Item Ledger. | 2026-07-12 (`cascade-eval-first`) |
...
| Key | Default | Description |
|-----|---------|-------------|
| `phantom_detection_mode` | `commons_freshness` | How the manager detects dead/stalled persona sessions. Allowed: `commons_freshness` (**default since 2026-07-20** — manager polls `commons_who` for stale `last_post_ts` on each wake event, where the wake comes from the standing arbiter + per-session Stop-hook), `bridge_file_mtime`. ⛔ `heartbeat_handling_via_external_scheduler` is **RETIRED (Rick GO 2026-06-29)** and must NOT be selected — it names the per-cascade daemon that playbook §6.4 forbids launching. It was the DEFAULT here until 2026-07-20, which meant a manager resolving config from this table and stopping there was instructed to stand up the exact daemon §6.4 prohibits. The legacy `heartbeat_ping` was REMOVED 2026-05-18 — incompatible with turn-based CC. |
| `stall_threshold_minutes` | `10` | Time without persona response before declaring phantom. |
| `phantom_reassignment_policy` | `park_and_escalate` | What happens when a persona is declared phantom. Allowed: `park_and_escalate` (section pauses, user decides — *retained as default on blast-radius grounds, NOT on capability grounds*), `respawn_same_persona` (manager spawns fresh — **available now**; the `spawn_sessions` MCP verb is live and playbook §Step 4 documents manager-side spawning), `swap_to_alternate_persona`. **Corrected 2026-07-20**: this row previously justified the default with *"required under current platform: manager cannot spawn new CC sessions"* and gated `respawn_same_persona` on *"requires v2 bounded-job infrastructure"*. Both statements were false as written — managers can spawn, and that infrastructure shipped. The default may still be the right call; the reason given for it was not. |
...
| Key | Default | Description |
|-----|---------|-------------|
| `heartbeat_daemon_kickoff_policy` | `none` — ⛔ **KEY RETIRED 2026-07-20** | 🗄️ **HISTORICAL. No live referent — do NOT launch any heartbeat daemon.** The standing arbiter + per-session Stop-hook are the waker (playbook §6.4, Rick GO 2026-06-29); there is no per-cascade daemon left to kick off, so there is nothing for this key to select. Retained as a record of the retired v1.1 shape only: `dual_independent` (Manager AND Observer each launched their own daemon — codified Run-4 prep where Rick caught that both needed launching), `manager_only` (legacy pre-v1.1), `centralized_orchestrator` (unbuilt v3 escape hatch). |
...
- **2026.07.20 (Tiffany 💍 — Workflow Steward; found by Clayton 😎, cascade manager)** — **Three retired-mechanism entries corrected. Commit HELD.** Surfaced during a live cascade on `skills-distillation`, by a Manager who resolved config from this table and noticed the defaults named things §6.4 forbids.
  ...
  2. **`phantom_reassignment_policy` rationale CORRECTED** — the default was justified by *"required under current platform: manager cannot spawn new CC sessions"* and `respawn_same_persona` was gated on *"requires v2 bounded-job infrastructure."* **Both false**: `spawn_sessions` is live and §Step 4 documents manager-side spawning. Default retained on blast-radius grounds; only the false reason was removed. *A default kept for a reason that has expired is a decision nobody is re-making.*
  3. **`heartbeat_daemon_kickoff_policy` RETIRED** — no live referent; there is no per-cascade daemon left to kick off. Fenced HISTORICAL.

  **The class, for the post-game**: the 2026-06-29 scheduler retirement and the `spawn_sessions` arrival each invalidated text in **more than one file**, and only the file someone happened to read got fixed. Clayton's proposed control — *grep for the retired mechanism's name at retirement time, not at next-reader time* — would have caught all of these at once. Empirical cost this run: the Workflow Steward raised a false blocking risk to the user **twice** off `plan-review-cascaded.md` prereq 4 (also corrected 2026-07-20) before reading §6.4. **Verified during the sweep**: `plan-review-cascaded-common.md` §Heartbeat Handling is **already correctly fenced** at section level and needed no change; the two remaining hits in changelog entries are accurate historical records and were deliberately left intact.
- **2026.07.12 (María 🌸 — DRAFT, awaiting Rick's approval)** — NEW **§Task-item ledger** config block (4 keys: `task_ledger_required`, `task_ledger_tiers`, `task_ledger_correlation_key`, `task_ledger_stale_threshold_minutes`) + new `kind` enum value `task_ledger_ready` (Step 0.4 output). Backs the new Manager MANDATE in `plan-review-cascaded-common.md` §Stepwise Task-Item Ledger: one store item per cascade step + one per section, correlated, statuses current — so cascade progress is pull-able instead of push-only. Empirical anchor: `cascade-eval-first` (2026-07-12). User directive: USER BROADCAST `255b682f`.
