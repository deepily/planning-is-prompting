...
1. **Read the pointer** — `io/mementos/<persona-slug>.md` (worker) or `.claude-memento.md` (self-`/clear`). The pointer **carries the current record's full content**, so a plain `Read` is already correct: **there is nothing to choose, no mtime tiebreak, no candidates to rank.**
   - 🔴 **THE SELF-`/clear` POINTER IS SHARED BY EVERY PERSONA IN THE REPO — CHECK ITS HEADER `session_id` IS YOURS BEFORE YOU TRUST A WORD OF IT.** `.claude-memento.md` is one file per repo, so it may be **somebody else's memento**. Measured 2026-08-31 (row `8f5dc4df`): the pointer changed hands **four times in 37 minutes**, and the author of the evidence document was locked out of `self_respin` while writing it. **Your own RECORD is per-persona** — `.claude-memento-<persona-slug>-<sid8>.md` — so when the header is not yours, read your record directly instead.
   - ⚠️ **This line used to say `.claude-memento.md` with no caveat, and that is the sentence the defect travelled on** — a standing instruction telling every seat to read a shared file as though it were its own. The caveat above is the part that is actually in force.
   - 🔴 **DO NOT re-introduce a `.claude-memento-<persona-slug>.md` pointer here until the WRITER creates one.** Step 3 of the per-persona change (`072ef7e`) was **reverted at `d4f6c29`**, so `memento_io.py` still writes the shared name (`pointer_rel_path`, the `slot == "root"` branch). Between 2026-08-31 and 2026-09-01 this step named a per-persona pointer that **no code ever created**, and told readers the fallback would retire on their next write — it does not. Ordering rule, from row `8f5dc4df`: **this command lands WITH or AFTER the writer, never before it.**
