...
1. **Read the pointer** — `io/mementos/<persona-slug>.md` (worker) or `.claude-memento-<persona-slug>.md` (self-`/clear`). The pointer **carries the current record's full content**, so a plain `Read` is already correct: **there is nothing to choose, no mtime tiebreak, no candidates to rank.**
   - ✅ **BOTH POINTERS ARE PER-PERSONA NOW, so the shared-file hazard this step warned about for two days is GONE BY CONSTRUCTION.** Step 3 (`072ef7e`) was reverted at `d4f6c29`; Rick authorised re-landing it as a patch on 2026-09-02 and it is in. `memento_io.py` writes `.claude-memento-<persona-slug>.md`, so **the file you are told to read is a file only you write.** No header check is needed to establish ownership — the name does it.
   - ⚠️ **`.claude-memento.md` — the old shared pointer — is LEGACY, read-only and no longer maintained.** If you find one, it is a leftover: it was last written before the re-land and **will drift further from the truth every time anybody writes a memento.** Do not read it, and do not "fix" it by regenerating — `regenerate-pointer` now writes the per-persona name and will leave the stale shared file exactly where it is. Delete it, or ignore it.
   - 🔴 **IF YOUR PER-PERSONA POINTER IS ABSENT, DO NOT FALL BACK TO THE SHARED FILE.** Your RECORD is per-persona and immutable — `.claude-memento-<persona-slug>-<sid8>.md` — so read it directly, or regenerate the pointer (§ below). Falling back to `.claude-memento.md` is how you end up reading a memento two sessions stale with nothing warning you: measured 2026-09-02, two lupin seats had a *present* per-persona pointer naming an *old* record, so the fallback never fired and they resolved cleanly to the wrong thing.
   - ⚠️ **Ordering rule, from row `8f5dc4df`, and it is why this paragraph changed in the same commit as the writer:** this command lands **WITH or AFTER** the writer, never before it. Between 2026-08-31 and 2026-09-01 this step named a per-persona pointer that no code created, and told readers the fallback would retire on their next write when it never would. **Do not edit this step to describe a `memento_io.py` that has not shipped.**
